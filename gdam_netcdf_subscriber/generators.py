...
def generate_global_id(configs, dataset):
    ...
    global_id = '%s-%04d%02d%02dT%02d%02d' % (
        glider_id,
        start_time.year,
        start_time.month,
        start_time.day,
...
def generate_time_bounds(dataset):
    ...
    bounds['history'] = (
        "%s: %s" % (now_time.isoformat(), __file__)
    )
    ...
    bounds['date_created'] = now_time.isoformat()
    bounds['date_issued'] = now_time.isoformat()
    bounds['date_modified'] = now_time.isoformat()
...
def generate_global_attributes(configs, dataset):
    ...
    global_attributes['history'] = (
        "%s: %s" % (datetime.utcnow().isoformat(), " ".join(sys.argv))
    )
